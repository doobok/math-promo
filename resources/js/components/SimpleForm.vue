        ...
        <template v-if="!sended">
            <div class="w-full">
                <div class="w-full p-2 relative mb-1">
                    <label for="name" class="inline-block mb-1 font-medium">{{ $ml.get('name') }}</label>
                    <input
                        type="text" name="name"
                        v-model="name"
                ...
                <div class="w-full p-2 relative mb-3">
                    <label for="phone" class="inline-block mb-1 font-medium">{{ $ml.get('phone') }}</label>
                    <input
                ...
                <p class="text-xs text-gray-600 sm:text-sm pt-2">
                    {{ $ml.get('simple_form_promo') }}
                </p>
            </div>
        </template>
        <template v-else>
            <div class="grid grid-cols-1 justify-items-center place-content-start text-center">
                <svg xmlns="http://www.w3.org/2000/svg" class="text-primary-500 h-32 m-4" viewBox="0 0 24 24"
                     fill="currentColor">
                    <path
                ...
                this.$store.dispatch('SEND_LEAD', {
                    ...
                    if (res.msg) {
                        ...
                        gtag('event', 'sendPhone', {
                            ...
                            'event_label': 'PROMO ' + this.slug + ' ПРОБНЫЙ'
                        });
                        ...
                        this.loading = false;
                        this.errorshow = true;
                        this.error = 'Виникла помилка. Дані не вдалося надіслати. Спробуйте повторити спробу трохи згодом.';
                        // console.log(res);
                    }
                })
            }
